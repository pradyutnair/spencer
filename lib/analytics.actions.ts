...
export function fetchExpenditure(transactions: any, dateRange: any, currency: string) {
  ...
  transactions = transactions.filter((transaction: { currency: string; }) => transaction.currency === currency);
  ...
  let percentageDifference = previousExpenditure !== 0
    ? ((Math.abs(currentExpenditure) - Math.abs(previousExpenditure)) / Math.abs(previousExpenditure)) * 100
...
export function fetchIncome(transactions: any, dateRange: any, currency: string){
  // Determine the date range, defaulting to the start of the month to today
  const from = dateRange?.from || startOfMonth(new Date());
  const to = dateRange?.to || new Date();
